...
export function usePartidas() {
    ...
    const [isLoadingPartidas, setIsLoadingPartidas] = useState(true);
    const fetchPartidas = useCallback(async () => {
        ...
        try {
            ...
            const formattedPartidas = data
                .filter((p) => p.tipo === "RITO")
                .map((p) => ({
                ...p,
    ...
    const updatePartidaName = async (partidaId, newName) => {
        try {
