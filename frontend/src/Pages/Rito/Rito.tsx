...
import { Recycling, Edit, Save } from '@mui/icons-material';
import { PageWrapper } from '../../Components/PageWrapper';
import { BoxMembros, BoxRito, WhiteTextField } from './Rito.styles'; // Manter os customizados
...
const highlightRed = keyframes`
    ...
    100% { background-color: transparent; }
`;
export const AnimatedTableRow = styled(TableRow)<{ highlight?: 'green' | 'red' }>`
    &.highlight-green {
...
export const RitoPage: React.FC = () => {
  ...
  const [isResettingRoom, setIsResettingRoom] = useState<number | null>(null);
  ...
  const [roomToReset, setRoomToReset] = useState<number | null>(null);

  const [highlightedRow, setHighlightedRow] = useState<number | null>(null);
  const [highlightType, setHighlightType] = useState<'green' | 'red' | null>(null);

  const [editingPartidaId, setEditingPartidaId] = useState<number | null>(null);
  const [editingPartidaName, setEditingPartidaName] = useState<string>('');


  // --- Funções de API ---

  const fetchAllMembers = useCallback(async (): Promise<void> => {
  ...
  const startEditingPartidaName = (partida: Partida) => {
  ...
  const savePartidaName = async (partidaId: number) => {
  ...
  const movePlayerToRoom = async (player: Player, currentPartidaId: number | null, targetPartidaId: number) => {
  ...
  const removePlayerFromRoom = async (player: Player, currentPartidaId: number) => {
    const originalPartidas = [...partidas];
    const originalAllPlayers = [...allPlayers];
  ...
  const handleReset = (partidaId: number | null) => {
  ...
  const confirmResetAllRooms = async () => {
    ...
    const allPlayersToReset: Player[] = partidas.flatMap(p => p.membros);
  ...
  return (
    <PageWrapper>
        ...
        <Box sx={{
          ...
          <Box sx={{
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%', alignItems: 'center' }}>
              <Typography variant="h6" sx={{ color: 'white', whiteSpace: 'nowrap', flexGrow: 1 }}>Lixos Disponíveis</Typography>
              <Button
                variant="contained"
                color="error"
                ...
                aria-label="Resetar todas as salas"
              >
                Resetar Todas
              </Button>
            </Box>
            <TextField
              ...
              InputProps={{ sx: { color: 'white', '& fieldset': { borderColor: 'rgba(255, 255, 255, 0.5)' } } }}
            />
          </Box>
          <AvailablePlayersTable
            ...
            getClassBackgroundColor={getClassBackgroundColor}
      ...
      <BoxRito>
        {isLoadingPartidas ? (
          <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%' }}>
            <CircularProgress />
            <Typography sx={{ ml: 2, color: 'white' }}>Carregando partidas...</Typography>
          </Box>
        ) : (
          partidas.map((partida) => (
            <Paper
              key={partida.id}
              elevation={6} // Sombra mais proeminente para cada sala
              sx={{
            ...
            >
              <RoomHeader
                isEditing={editingPartidaId === partida.id}
                name={partida.nome}
                membersCount={partida.membros.length}
                maxMembers={partida.capacidadeMaximaJogadores}
                editingName={editingPartidaName}
                onEdit={() => startEditingPartidaName(partida)}
                onChangeName={(e: React.ChangeEvent<HTMLInputElement>) => setEditingPartidaName(e.target.value)}
                onSave={() => savePartidaName(partida.id)}
                onCancel={cancelEditingPartidaName}
                onReset={() => resetRoom(partida.id)}
                isSavingName={isSavingName}
              />

              {/* Tabela de Membros da Sala */}
              <RoomMembersTable
                membros={partida.membros}
                partidas={partidas}
      ...
      <ConfirmDialog
        ...
        title={roomToReset === -1 ? "Confirmar Reset de Todas as Salas" : `Confirmar Reset da Sala`}
        message={roomToReset === -1
          ? "Tem certeza que deseja remover TODOS os membros de TODAS as salas? Esta ação não pode ser desfeita."
          : `Tem certeza que deseja remover todos os membros da sala "${partidas.find(p => p.id === roomToReset)?.nome || 'desconhecida'}"? Esta ação não pode ser desfeita.`}
        onCancel={cancelResetDialog}
        onConfirm={roomToReset === -1 ? confirmResetAllRooms : confirmResetRoom}
        isResettingAllRooms={isResettingAllRooms}
